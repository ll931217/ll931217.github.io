  ...
  #searchbar
    form(action="https://www.google.com/search", method="GET", @submit.prevent="submit")
      label(for="search")
        span.right >
      .input-command
        input(type="text", name="q", autocomplete="off", v-model="text", @keydown.tab.prevent="autocomplete", autofocus)#search
  ...
  export default {
          ...
          if (checkCmd.test(this.text)) {
            if (arr[1] === 'add') {
              if (arr.length === 6) {
                ...
                let first = arr[2].split('=')
                let second = arr[3].split('=')
                let name = ''
                let url = ''
                if (first[0] === '--name') {
              ...
              if (arr.length === 3) {
                let name = arr[2].split('=')
                let n = name[1].replace(/['"]+/g, '')
                if (name[0] === '--name') {
    ...
    watch: {
      text() {
        ...
        if (this.text !== '') {
          let regex = new RegExp(`^${this.text}*`)
          this.suggestion = this.commands.find(v => regex.test(v))
        }

        if (this.errCode !== 0) this.errCode = 0
      }
    },
    computed: {
      ...mapGetters({
